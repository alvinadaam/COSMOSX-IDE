// Legacy Live Preview System - Fixed and Compatible
// This is a clean version that works with the modern preview HTML structure
...
export class LivePreview {
    ...
    setupRealTimeUpdates() {
        // Listen for editor changes - check for Monaco editor API
        if (window.editor && typeof window.editor.onDidChangeModelContent === 'function') {
            ...
            // Alternative Monaco API approach
            ...
            // Fallback: poll for changes or use different event system
            console.warn('Monaco editor change events not available, using fallback');
            this.setupFallbackUpdates();
        }
    }

    // Fallback update system when Monaco events aren't available
    setupFallbackUpdates() {
        let lastContent = '';
        
        // Poll for changes every 2 seconds when playing
        this.updateInterval = setInterval(() => {
            if (this.isPlaying) {
                let currentContent = '';
                if (window.ide && window.ide.editorManager && typeof window.ide.editorManager.getValue === 'function') {
                    currentContent = window.ide.editorManager.getValue();
    ...
    renderChoices(renderableState) {
        const choices = renderableState.choices || [];
        ...
        const choicesHTML = choices.map((choice, index) => `
            <button class="choice-button" data-choice="${choice.index}">
                ${choice.text}
            </button>
        `).join('');
